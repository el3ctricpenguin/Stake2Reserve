...
describe("Stake2Reserve", () => {
    ...
    const deployedContractAndRegisteredShopProperty = async () => {
        ...
        const courses = [
            {
                ...
                cancelFee: 100,
                imageURLs: ["https://i.imgur.com/4dr7xZo.jpeg", "https://i.imgur.com/zWwZ1Bm.jpeg"],
            },
            {
                name: "delicious sushi #2",
                cancelFee: 200,
                imageURLs: ["https://i.imgur.com/4dr7xZo.jpeg", "https://i.imgur.com/zWwZ1Bm.jpeg"],
            },
        ];
        ...
        const description =
            "ZenBite Sushi is a fictional sushi restaurant with a serene garden ambiance. Their menu includes both traditional and innovative sushi rolls, prepared by expert chefs at an open bar, providing a unique dining experience.";
    ...
    const deployedContractAndRegisteredShopPropertyAndReservedSome = async () => {
        ...
        await contract
            ...
            .reserve(
                owner.address,
                new Date(Date.UTC(2023, 10 - 1, 3, 12 + 4, 30, 0)).getTime() / 1000,
                new Date(Date.UTC(2023, 10 - 1, 3, 13 + 4, 30, 0)).getTime() / 1000,
                2,
                0
            );
        await contract
            ...
            .reserve(
                owner.address,
                new Date(Date.UTC(2023, 10 - 1, 6, 10 + 4, 0, 0)).getTime() / 1000,
                new Date(Date.UTC(2023, 10 - 1, 6, 11 + 4, 0, 0)).getTime() / 1000,
                2,
                1
            );
        return { owner, otherAccount, contract, usdc };
    ...
    describe("Week and Time", () => {
        ...
        describe("Time", () => {
            it("should return correct diff time #1", async () => {
                const { contract } = await loadFixture(deployContract);
                expect(await contract.getTime(new Date(Date.UTC(2023, 9 - 1, 23, 0 + 4, 45, 40)).getTime() / 1000)).to.equal(
                    new Date(Date.UTC(1970, 1 - 1, 1, 0, 45, 40)).getTime() / 1000
                );
            });
            it("should return correct diff time #2", async () => {
                const { contract } = await loadFixture(deployContract);
                expect(await contract.getTime(new Date(Date.UTC(2023, 2 - 1, 3, 5 + 4, 15, 0)).getTime() / 1000)).to.equal(
                    new Date(Date.UTC(1970, 1 - 1, 1, 5, 15, 0)).getTime() / 1000
                );
            });
            it("should return correct diff time #3", async () => {
                const { contract } = await loadFixture(deployContract);
                expect(await contract.getTime(new Date(Date.UTC(2024, 9 - 1, 4, 3 + 4, 34, 21)).getTime() / 1000)).to.equal(
                    new Date(Date.UTC(1970, 1 - 1, 1, 3, 34, 21)).getTime() / 1000
                );
            });
            it("should return correct diff time #4", async () => {
                const { contract } = await loadFixture(deployContract);
                expect(await contract.getTime(new Date(Date.UTC(2025, 11 - 1, 2, 17 + 4, 16, 56)).getTime() / 1000)).to.equal(
                    new Date(Date.UTC(1970, 1 - 1, 1, 17, 16, 56)).getTime() / 1000
                );
            });
        });
    });
    describe("Reservation", () => {
        describe("require", () => {
            it("should be reverted because of wrong week day", async () => {
                ...
                await expect(contract.reserve(owner.address, reservationStartTime, reservationEndTime, 2, 1)).to.be.revertedWith(
            ...
            it("should be reverted because courseId doesn't exist", async () => {
                ...
                await expect(contract.reserve(owner.address, reservationStartTime, reservationEndTime, 2, 3)).to.be.revertedWith(
            ...
            it("should be reverted because reserving past", async () => {
                // could be commented (for Demo)
                const { owner, contract, usdc } = await loadFixture(deployedContractAndRegisteredShopProperty);
                ...
                await expect(contract.reserve(owner.address, reservationStartTime, reservationEndTime, 2, 0)).to.be.revertedWith(
                    "startingTime is past"
                );
            });
    ...
    describe("Check Out", () => {
        describe("setPaymentAmount", () => {
            it("should be reverted because of wrong msg.sender", async () => {
                ...
                await expect(contract.connect(otherAccount).setPaymentAmount(0, 200)).to.be.revertedWith(
        ...
        describe("getNoShowNFTs", () => {
            ...
            it("should return NFTs(tokenId: 0,3)", async () => {
                ...
                await contract
                    ...
                    .reserve(
                        owner.address,
                        new Date(Date.UTC(2023, 10 - 1, 1, 10 + 4, 0, 0)).getTime() / 1000,
                        new Date(Date.UTC(2023, 10 - 1, 1, 11 + 4, 0, 0)).getTime() / 1000,
                        2,
                        1
                    );
